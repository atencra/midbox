function [cc, location] = mid2_fsi(strf, SpecFile, T1, T2, spet, Trig, Fss, SPL, MdB)
%
...
% pause;
         ...
         if ( size(stimdnsmp,1)==size(strf,1) & size(stimdnsmp,2)==size(strf,2) )
            ...
            strfvec = reshape(strf, size(strf,1)*size(strf,2), 1); % N x 1
				...
				c = corrcoef( stimvec(:), strfvec(:) ); % c will be a 2x2 matrix

            cc = [cc; c(1,2)]; % take the off diagonal element
