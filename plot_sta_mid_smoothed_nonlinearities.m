function plot_sta_mid_smoothed_nonlinearities(mid, saveit)
...
%    he = errorbar(x_v1, fx_v1, fx_se_v1, 'ko-');
   plot(x_v1, smooth3p(fx_v1), 'k-', 'linewidth', 2);
   plot([min(x_sta) max(x_sta)], [pspike pspike], 'k:');
   xlim([-7.5 7.5]);
   set(gca,'xtick', [-7.5 -5 -2.5 0 2.5 5 7.5], ...
      'xticklabel', [-7.5 -5 -2.5 0 2.5 5 7.5]);
...
%    he = errorbar(x_v2, fx_v2, fx_se_v2, 'ko-');
   plot(x_v2, smooth3p(fx_v2), 'k-', 'linewidth', 2);
   plot([min(x_sta) max(x_sta)], [pspike pspike], 'k:');
   xlim([-6 6]);
