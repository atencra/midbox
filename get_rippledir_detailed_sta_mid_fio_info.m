function [mid, info] = get_rippledir_detailed_sta_mid_fio_info(filestruct)
...
for i = 1:length(filestruct)
   ...
   % assign initial struct array fields
   if isfield(filestruct, 'exp')
      mid(i).exp = filestruct(i).exp;
      info(i).exp = filestruct(i).exp;
   else
      mid(i).exp = 'unknown';
      info(i).exp = 'unknown';
   end

   if isfield(filestruct, 'site')
      mid(i).site = filestruct(i).site;
      info(i).site = filestruct(i).site;
   else
      mid(i).site = 9999;
      info(i).site = 9999;
   end

   if isfield(filestruct, 'chan')
      mid(i).chan = filestruct(i).chan;
      info(i).chan = filestruct(i).chan;
   else
      mid(i).chan = 9999;
      info(i).chan = 9999;
   end

   if isfield(filestruct, 'model')
      mid(i).model = filestruct(i).model;
      info(i).model = filestruct(i).model;
   else
      mid(i).model = 9999;
      info(i).model = 9999;
   end

   if isfield(filestruct, 'depth')
      mid(i).depth = filestruct(i).depth;
      info(i).depth = filestruct(i).depth;
   else
      mid(i).depth = 9999;
      info(i).depth = 9999;
   end

   if isfield(filestruct, 'position')
      mid(i).position = filestruct(i).position;
      info(i).position = filestruct(i).position;
   else
      mid(i).position = 9999;
      info(i).position = 9999;
   end

   if isfield(filestruct, 'stim')
      mid(i).stim = filestruct(i).stim;
      info(i).stim = filestruct(i).stim;
   else
      mid(i).stim = 9999;
      info(i).stim = 9999;
   end

   if isfield(filestruct, 'atten')
      mid(i).atten = filestruct(i).atten;
      info(i).atten = filestruct(i).atten;
   else
      mid(i).atten = 9999;
      info(i).atten = 9999;
   end
