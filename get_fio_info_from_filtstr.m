function [fio, projinfo] = get_fio_info_from_filtstr(filtstr, stimulus, locator_mtx)
% get_fio_info_from_filtstr   Nonlinearities and info from stimulus, spikes, and filters
%
% [fio, projinfo] = get_fio_info_from_filtstr(filtstr, stimulus, locator_mtx)
% ------------------------------------------------------------------------
%
...
for i = 1:length(filtstr)
   ...
   numtbins = filtstr(i).numtbins;
   numfbins = filtstr(i).numfbins;
   ...
   if ( ~isempty(locator_mtx) )
      ...
      if ( length(d) > 1 )
         error('More than one file with that unit number.');
      elseif ( length(d) == 0 )
   ...
   %--------------------------------------------------------------------
   %   Initial save of unit basic information
   %--------------------------------------------------------------------

	if ( isfield(filtstr, 'exp') )
	   projinfo(i).exp = filtstr(i).exp;
		projinfo(i).site = filtstr(i).site;
		projinfo(i).chan = filtstr(i).chan;
		projinfo(i).model = filtstr(i).model;
		projinfo(i).depth = filtstr(i).depth;
		projinfo(i).position = filtstr(i).position;
		projinfo(i).stim = filtstr(i).stim;
		projinfo(i).atten = filtstr(i).atten;
		projinfo(i).spl = filtstr(i).spl;
		projinfo(i).sm = filtstr(i).sm;
		projinfo(i).tm = filtstr(i).tm;
		projinfo(i).mdb = filtstr(i).mdb;

	   fio(i).exp = filtstr(i).exp;
		fio(i).site = filtstr(i).site;
		fio(i).chan = filtstr(i).chan;
		fio(i).model = filtstr(i).model;
		fio(i).depth = filtstr(i).depth;
		fio(i).position = filtstr(i).position;
		fio(i).stim = filtstr(i).stim;
		fio(i).atten = filtstr(i).atten;
		fio(i).spl = filtstr(i).spl;
		fio(i).sm = filtstr(i).sm;
		fio(i).tm = filtstr(i).tm;
		fio(i).mdb = filtstr(i).mdb;
	else
	   projinfo(i).exp = [];
		projinfo(i).site = [];
		projinfo(i).chan = [];
   ...
   if ( size(filtstr(i).mtx_sta,2)==4 && size(filtstr(i).mtx_v1,2)==4 && size(filtstr(i).mtx_v2,2)==4 )
...
%       ifrac12_std_test, 'MID1 and MID2 Info Analysis');
   ...
   end % if ( size(filtstr(i).mtx_sta,2)==4 && ...
