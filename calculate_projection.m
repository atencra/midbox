function proj = calculate_projection(mid, spk, trigger, sprfile)
% calculate_projection - compute projection values for STA, MID1, and
%    MID2. 
%
...
for i = 1:length(mid)
   ...
   % First make the calculations for the mean filters:
   ...
   [x1x2rand_mean, dummy] = get_mid1_mid2_projection(mid1, mid2, sprfile, 0.000, 0.100, spetrand, trigger, fs, spl, mdb);


   % First we want to find the p(proj onto mid1 & mid2 | spike), so use the original spike
   % train and the original filter
   [x1x2spk_mean, dummy] = get_mid1_mid2_projection(mid1, mid2, sprfile, 0.000, 0.100, spet, trigger, fs, spl, mdb);
   ...
   [x1xrandspk_mean, dummy] = get_mid1_mid2_projection(mid1, mid2_rand, sprfile, 0.000, 0.100, spet, trigger, fs, spl, mdb);


   % Now we do it for the filters that were computed for different segments of the data:
   ...
   [x1x2rand_part1, dummy] = get_mid1_mid2_projection(mid1, mid2, sprfile, 0.000, 0.100, spetrand, trigger, fs, spl, mdb);
   [x1x2spk_part1, dummy] = get_mid1_mid2_projection(mid1, mid2, sprfile, 0.000, 0.100, spet, trigger, fs, spl, mdb);
   [x1xrandspk_part1, dummy] = get_mid1_mid2_projection(mid1, mid2_rand, sprfile, 0.000, 0.100, spet, trigger, fs, spl, mdb);
   ...
   [x1x2rand_part2, dummy] = get_mid1_mid2_projection(mid1, mid2, sprfile, 0.000, 0.100, spetrand, trigger, fs, spl, mdb);
   [x1x2spk_part2, dummy] = get_mid1_mid2_projection(mid1, mid2, sprfile, 0.000, 0.100, spet, trigger, fs, spl, mdb);
   [x1xrandspk_part2, dummy] = get_mid1_mid2_projection(mid1, mid2_rand, sprfile, 0.000, 0.100, spet, trigger, fs, spl, mdb);
   ...
   [x1x2rand_part3, dummy] = get_mid1_mid2_projection(mid1, mid2, sprfile, 0.000, 0.100, spetrand, trigger, fs, spl, mdb);
   [x1x2spk_part3, dummy] = get_mid1_mid2_projection(mid1, mid2, sprfile, 0.000, 0.100, spet, trigger, fs, spl, mdb);
   [x1xrandspk_part3, dummy] = get_mid1_mid2_projection(mid1, mid2_rand, sprfile, 0.000, 0.100, spet, trigger, fs, spl, mdb);
   ...
   [x1x2rand_part4, dummy] = get_mid1_mid2_projection(mid1, mid2, sprfile, 0.000, 0.100, spetrand, trigger, fs, spl, mdb);
   [x1x2spk_part4, dummy] = get_mid1_mid2_projection(mid1, mid2, sprfile, 0.000, 0.100, spet, trigger, fs, spl, mdb);
   [x1xrandspk_part4, dummy] = get_mid1_mid2_projection(mid1, mid2_rand, sprfile, 0.000, 0.100, spet, trigger, fs, spl, mdb);
   ...
   proj(i).trigger = trigger(:);
   proj(i).spetrand = spetrand(:);
   proj(i).randspklocation = randspklocation;
   proj(i).spklocation = spklocation;


   proj(i).xstarand_mean = xstarand_mean;
   proj(i).xstaspk_mean = xstaspk_mean;

   proj(i).xstarand_part1 = xstarand_part1;
   proj(i).xstaspk_part1 = xstaspk_part1;

   proj(i).xstarand_part2 = xstarand_part2;
   proj(i).xstaspk_part2 = xstaspk_part2;

   proj(i).xstarand_part3 = xstarand_part3;
   proj(i).xstaspk_part3 = xstaspk_part3;

   proj(i).xstarand_part4 = xstarand_part4;
   proj(i).xstaspk_part4 = xstaspk_part4;


   proj(i).x1x2rand_mean = x1x2rand_mean;
   proj(i).x1x2spk_mean = x1x2spk_mean;
   proj(i).x1xrandspk_mean = x1xrandspk_mean;

   proj(i).x1x2rand_part1 = x1x2rand_part1;
   proj(i).x1x2spk_part1 = x1x2spk_part1;
   proj(i).x1xrandspk_part1 = x1xrandspk_part1;

   proj(i).x1x2rand_part2 = x1x2rand_part2;
   proj(i).x1x2spk_part2 = x1x2spk_part2;
   proj(i).x1xrandspk_part2 = x1xrandspk_part2;

   proj(i).x1x2rand_part3 = x1x2rand_part3;
   proj(i).x1x2spk_part3 = x1x2spk_part3;
   proj(i).x1xrandspk_part3 = x1xrandspk_part3;

   proj(i).x1x2rand_part4 = x1x2rand_part4;
   proj(i).x1x2spk_part4 = x1x2spk_part4;
   proj(i).x1xrandspk_part4 = x1xrandspk_part4;
