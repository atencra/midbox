...
fsi_mean_fsi2_cdf ( isnan( fsi_mean_fsi2_cdf ) ) = 0;
fsi_mean_fsi2_abs_cdf ( isnan( fsi_mean_fsi2_abs_cdf ) ) = 0;
fsi_mean_fsi2_right_abs_cdf ( isnan( fsi_mean_fsi2_right_abs_cdf ) ) = 0;
fsi_mean_fsi2_left_abs_cdf ( isnan( fsi_mean_fsi2_left_abs_cdf ) ) = 0;
...
% variables to hold mid2 data
fsi_mean_fsi2_cdf_pop = cell(1,length(boundary)-1);
fsi_mean_fsi2_abs_cdf_pop = cell(1,length(boundary)-1);
fsi_mean_fsi2_right_abs_cdf_pop = cell(1,length(boundary)-1);
fsi_mean_fsi2_left_abs_cdf_pop = cell(1,length(boundary)-1);

fsi_mean_fsi2_cdf_mn = zeros(1,length(boundary)-1);
fsi_mean_fsi2_abs_cdf_mn = zeros(1,length(boundary)-1);
fsi_mean_fsi2_right_abs_cdf_mn = zeros(1,length(boundary)-1);
fsi_mean_fsi2_left_abs_cdf_mn = zeros(1,length(boundary)-1);

fsi_mean_fsi2_cdf_se = zeros(1,length(boundary)-1);
fsi_mean_fsi2_abs_cdf_se = zeros(1,length(boundary)-1);
fsi_mean_fsi2_right_abs_cdf_se = zeros(1,length(boundary)-1);
fsi_mean_fsi2_left_abs_cdf_se = zeros(1,length(boundary)-1);
...
for i = 1:length(boundary)-1
   ...
   fsi_mean_fsi2_cdf_pop{i} = fsi_mean_fsi2_cdf(index{i});
   fsi_mean_fsi2_abs_cdf_pop{i} = fsi_mean_fsi2_abs_cdf(index{i});
   fsi_mean_fsi2_right_abs_cdf_pop{i} = fsi_mean_fsi2_right_abs_cdf(index{i});
   fsi_mean_fsi2_left_abs_cdf_pop{i} = fsi_mean_fsi2_left_abs_cdf(index{i});

   fsi_mean_fsi2_cdf_mn(i) = mean( fsi_mean_fsi2_cdf(index{i}) );
   fsi_mean_fsi2_abs_cdf_mn(i) = mean( fsi_mean_fsi2_abs_cdf(index{i}) );
   fsi_mean_fsi2_right_abs_cdf_mn(i) = mean( fsi_mean_fsi2_right_abs_cdf(index{i}) );
   fsi_mean_fsi2_left_abs_cdf_mn(i) = mean( fsi_mean_fsi2_left_abs_cdf(index{i}) );

   fsi_mean_fsi2_cdf_se(i) = std( fsi_mean_fsi2_cdf(index{i}) ) / sqrt( length( fsi_mean_fsi2_cdf(index{i}) ) );
   fsi_mean_fsi2_abs_cdf_se(i) = std( fsi_mean_fsi2_abs_cdf(index{i}) ) / sqrt( length( fsi_mean_fsi2_abs_cdf(index{i}) ) );
   fsi_mean_fsi2_right_abs_cdf_se(i) = std( fsi_mean_fsi2_right_abs_cdf(index{i}) ) / sqrt( length( fsi_mean_fsi2_right_abs_cdf(index{i}) ) );
   fsi_mean_fsi2_left_abs_cdf_se(i) = std( fsi_mean_fsi2_left_abs_cdf(index{i}) ) / sqrt( length( fsi_mean_fsi2_left_abs_cdf(index{i}) ) );
...
data1(1).title = 'MID1 FSI CDF';
data1(1).position = position;
data1(1).data = fsi_mean_fsi1_cdf;
data1(1).boundary = boundary;
data1(1).depth = depth;
data1(1).pop = fsi_mean_fsi1_cdf_pop;
data1(1).mn = fsi_mean_fsi1_cdf_mn;
data1(1).se = fsi_mean_fsi1_cdf_se;
data1(1).ytick = [0:0.01:0.05];
data1(1).yax = [0 0.05];

data1(2).title = 'MID1 FSI ABS CDF';
data1(2).position = position;
data1(2).data = fsi_mean_fsi1_abs_cdf;
data1(2).boundary = boundary;
data1(2).depth = depth;
data1(2).pop = fsi_mean_fsi1_abs_cdf_pop;
data1(2).mn = fsi_mean_fsi1_abs_cdf_mn;
data1(2).se = fsi_mean_fsi1_abs_cdf_se;
data1(2).ytick = [0:0.01:0.05];
data1(2).yax = [0 0.05];

data1(3).title = 'MID1 FSI Right ABS CDF';
data1(3).position = position;
data1(3).data = fsi_mean_fsi1_right_abs_cdf;
data1(3).boundary = boundary;
data1(3).depth = depth;
data1(3).pop = fsi_mean_fsi1_right_abs_cdf_pop;
data1(3).mn = fsi_mean_fsi1_right_abs_cdf_mn;
data1(3).se = fsi_mean_fsi1_right_abs_cdf_se;
data1(3).ytick = [0:0.01:0.05];
data1(3).yax = [0 0.05];

data1(4).title = 'MID1 FSI Left ABS CDF';
data1(4).position = position;
data1(4).data = fsi_mean_fsi1_left_abs_cdf;
data1(4).boundary = boundary;
data1(4).depth = depth;
data1(4).pop = fsi_mean_fsi1_left_abs_cdf_pop;
data1(4).mn = fsi_mean_fsi1_left_abs_cdf_mn;
data1(4).se = fsi_mean_fsi1_left_abs_cdf_se;
data1(4).ytick = [0:0.01:0.05];
data1(4).yax = [0 0.05];


data2(1).title = 'MID2 FSI CDF';
data2(1).position = position;
data2(1).data = fsi_mean_fsi2_cdf;
data2(1).boundary = boundary;
data2(1).depth = depth;
data2(1).pop = fsi_mean_fsi2_cdf_pop;
data2(1).mn = fsi_mean_fsi2_cdf_mn;
data2(1).se = fsi_mean_fsi2_cdf_se;
data2(1).ytick = [0:0.01:0.05];
data2(1).yax = [0 0.05];

data2(2).title = 'MID2 FSI ABS CDF';
data2(2).position = position;
data2(2).data = fsi_mean_fsi2_abs_cdf;
data2(2).boundary = boundary;
data2(2).depth = depth;
data2(2).pop = fsi_mean_fsi2_abs_cdf_pop;
data2(2).mn = fsi_mean_fsi2_abs_cdf_mn;
data2(2).se = fsi_mean_fsi2_abs_cdf_se;
data2(2).ytick = [0:0.01:0.05];
data2(2).yax = [0 0.05];

data2(3).title = 'MID2 FSI Right ABS CDF';
data2(3).position = position;
data2(3).data = fsi_mean_fsi2_right_abs_cdf;
data2(3).boundary = boundary;
data2(3).depth = depth;
data2(3).pop = fsi_mean_fsi2_right_abs_cdf_pop;
data2(3).mn = fsi_mean_fsi2_right_abs_cdf_mn;
data2(3).se = fsi_mean_fsi2_right_abs_cdf_se;
data2(3).ytick = [0:0.01:0.05];
data2(3).yax = [0 0.05];

data2(4).title = 'MID2 FSI Left ABS CDF';
data2(4).position = position;
data2(4).data = fsi_mean_fsi2_left_abs_cdf;
data2(4).boundary = boundary;
data2(4).depth = depth;
data2(4).pop = fsi_mean_fsi2_left_abs_cdf_pop;
data2(4).mn = fsi_mean_fsi2_left_abs_cdf_mn;
data2(4).se = fsi_mean_fsi2_left_abs_cdf_se;
data2(4).ytick = [0:0.01:0.05];
data2(4).yax = [0 0.05];


% plot_depth_nonlinearity_params_circle_niceyticks(count, depth, data1);

% plot_depth_nonlinearity_params_circle_niceyticks(count, depth, data2);

plot_depth_fsi_circle_niceyticks(count, depth, data1, data2);
...
function plot_depth_fsi_circle_niceyticks(count, depth, data1, data2)
...
for i = 1:length(data1)
   ...
   ytick = data1(i).ytick;
   yax = data1(i).yax;
   titlestr = data1(i).title;

   subplot(length(data1),2,2*i-1);
   h = errorbar( depth, mn, se, 'ko-' );
   set(h, 'linewidth', 0.5, 'markersize', 3, 'markerfacecolor', [0.7 0.7 0.7], ...
   'markeredgecolor', [0.7 0.7 0.7]);
   ...
   ytick = data2(i).ytick;
   yax = data2(i).yax;
   titlestr = data2(i).title;

   subplot(length(data2),2,2*i);
   h = errorbar( depth, mn, se, 'ko-' );
   set(h, 'linewidth', 0.5, 'markersize', 3, 'markerfacecolor', [0.7 0.7 0.7], ...
   'markeredgecolor', [0.7 0.7 0.7]);
